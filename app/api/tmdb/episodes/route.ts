...
export async function GET(request: Request) {
  ...
  if (!tvId || !seasonNumber || !episodeNumber) {
    return new Response(JSON.stringify({ error: "tvId, seasonNumber, and episodeNumber parameters are required" }), {
      status: 400,
    })
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "Failed to fetch episode details" }), { status: 500 })
  }
}
