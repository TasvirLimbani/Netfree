...
const MovieDetail = () => {
  ...
  useEffect(() => {
    const tryuu = `https://net51.cc/pv/playlist.php?id=${id}`;
    const tryywrap = `https://api.allorigins.win/get?url=${encodeURIComponent(
      tryuu
    )}`;
  ...
  useEffect(() => {
    const fnlapi = `https://api.allorigins.win/get?url=${encodeURIComponent(
      `https://net51.cc${apifnal}`
    )}`;
    ...
    fetch(fnlapi)
      ...
      .then((data) => {
        ...
        if (m3u8Base64.startsWith(base64Prefix)) {
          ...
          const bucketUrl = decoded
            .toString()
            .split("CAPTIONS=NONE")[1]
            .split("/files")[0];
          setBucket(bucketUrl);
        } else {
  ...
  return (
    <>
      <div className={Style.transNav}>
        <img
          src="/assets/images/Netfree-full.png"
          alt="Netflix Logo"
          onClick={() => navigate("/")}
        />
      </div>
      <div className={Style.main}>
        <img src={`https://imgcdn.kim/pv/c/${id}.jpg`} alt="" />
          ...
          <div className={Style.bannerDetail}>
            ...
            <div style={{ display: "flex", gap: "15px" }}>
              ...
              <button
                ...
                className={`${Style.watchlistbtn} ${
                  watchlist.includes(id) ? Style.added : ""
                }`}
              >
